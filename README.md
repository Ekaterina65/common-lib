...
3.  **Validation**: A network of validators listens for this event and verifies its legitimacy.
...
This script plays the role of a **validator node's listening component**. It is responsible for Step 3 and initiating Step 4.
...
-   `EventScanner`: The core logic for monitoring the blockchain. It uses a `BlockchainConnector` to scan ranges of blocks for the target event (`TokensLocked`). It's designed to handle RPC node limitations by scanning in manageable chunks (`BLOCK_SCAN_RANGE`).
...
1.  The `CrossChainBridgeListener` starts its main loop.
...
4.  The `EventScanner` queries the source chain's RPC node for `TokensLocked` event logs.
5.  If events are found, the `CrossChainBridgeListener` iterates through them, passing each one to `EventHandler.process_event()`.
6.  The `EventHandler` simulates validation and the creation of a minting transaction for the destination chain.
7.  If all events in the block range are processed successfully, the `CrossChainBridgeListener` updates `last_scanned_block` and saves the new state to a file.
...
Once the requirements are installed and the `.env` file is configured, you can run the script. The entry point of the script (e.g., `main.py`) would look something like this:
